...
1. the JsonLang structure is Simple and Optimized. Its [structure](#structure) and [rules](./category/rules) have a shortcut to make your JSON in a small size.
...
Execute is the `Sync` version of JsonLang, use it to run all [builtin rules](./category/rules) and any [extended](./category/extend) `Sync` Rules
...
Execute is the `Async` version of JsonLang, use it to run all [builtin rules](./category/rules) and any [extended](./category/extend) `Sync` or `Async` Rules
